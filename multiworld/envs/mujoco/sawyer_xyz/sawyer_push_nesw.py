...
class SawyerPushAndReachXYEnv(MujocoEnv, Serializable, MultitaskEnv):
    ...
    def __init__(
            ...
            hide_goal=False,
            init_block_low=(-0.1, 0.5),
            init_block_high=(0.1, 0.7),
    ...
    def sample_goal_xyxy(self):
        ...
        instr_no = random.randint(0, 3)

        if instr_no == 0:
            puck = pos + np.array([0, 0.05])
        if instr_no == 1:
            puck = pos + np.array([0, -0.05])
        if instr_no == 2:
            puck = pos + np.array([0.05, 0])
        if instr_no == 3:
            puck = pos + np.array([-0.05, 0])
        #else:
    #        hand = self.fixed_hand_goal.copy()
        #    puck = self.fixed_puck_goal.copy()
        return np.hstack((hand, puck)), pos
    ...
    def reset(self):
        ...
        self._goal_xyxy, puck = self.sample_goal_for_rollout()
        self.set_goal_xyxy(self._goal_xyxy)
        self.set_puck_xy(puck)
        self.reset_mocap_welds()
