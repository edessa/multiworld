...
class SawyerPickAndPlaceEnv(MultitaskEnv, SawyerXYZEnv):
    ...
    def __init__(
            ...
            mocap_low=(-0.15, 0.58, 0.04),
            mocap_high=(0.1, 0.78, 0.2),
            action_scale=0.02,
...
#            self.sim.forward()
    ...
    def generate_uncorrected_env_goals(self, num_goals):
        ...
        #    pos[0] = self.INIT_HAND_POS[:2] #first elem. of pos is hand posiiton
        ...
        #                break
            for j in range(num_objs_in_hand, len(goals)):
            #    print(np.append(self.get_object_pos(random.randint(1, self.num_objects))[:2], 0.05))
